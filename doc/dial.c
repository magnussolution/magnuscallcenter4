...
static int dfltInitialSilence       = 1500;
...
static int amdStatusMC = 0;
char amdCause[256] = "", amdStatus[256] = "";

static void amd_exec(struct ast_channel *chan, int totalAnalysisTime, int maximumNumberOfWords)
{
	...
	ast_verb(3, "totalAnalysisTime [%d]  maximumNumberOfWords [%d]\n",totalAnalysisTime,  maximumNumberOfWords);
	...
	ast_verb(3,"amdStatus %s -> amdStatusMC = %d\n",amdStatus, amdStatusMC);
...
static void handle_frame(struct ast_dial *dial, struct ast_dial_channel *channel, struct ast_frame *fr, struct ast_channel *chan)
{
	if (fr->frametype == AST_FRAME_CONTROL) {
		...
		case AST_CONTROL_ANSWER:
			if (chan) {
				ast_verb(3, "%s answered 111 %s\n", ast_channel_name(channel->owner), ast_channel_name(chan));
			} else {
				ast_verb(3, "%s answered 2222\n", ast_channel_name(channel->owner));
			}
			AST_LIST_LOCK(&dial->channels);
		...
		case AST_CONTROL_PROGRESS:
			if (pbx_builtin_getvar_helper(channel->owner, "AMD")  )
			{
				int totalAnalysisTime = atoi(pbx_builtin_getvar_helper(channel->owner, "AMDtotalAnalysisTime"));
				int maximumNumberOfWords = atoi(pbx_builtin_getvar_helper(channel->owner, "AMDmaximumNumberOfWords"));
				ast_verb(3, "%s is making progress TRY AMD FROM PREDICTIVE\n", ast_channel_name(channel->owner));	
				amd_exec(channel->owner,totalAnalysisTime,maximumNumberOfWords);
					
				if (amdStatusMC == 1)
				{
					ast_verb(3,"HANGUP CALL BECAUSE AMD DETECT MACHINE\n");
					ast_channel_publish_dial(chan, channel->owner, channel->device, "CONGESTION");
					ast_hangup(channel->owner);
					channel->cause = AST_CAUSE_NORMAL_CIRCUIT_CONGESTION;
					channel->owner = NULL;
